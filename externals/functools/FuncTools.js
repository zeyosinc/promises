...
*/
function $callable(v) { return v && typeOf(v) == 'function'; }
...
function $get(first, prop) {
  ...
  var args = Array.from(arguments), rest = args.rest(2), next;
  if(rest.length == 0) return first[prop];
  if(['object', 'array'].contains(typeOf(first))) next = first[prop];
  if(typeOf(next) == 'function') next = first[prop]();
...
Function.implement({
  ...
  */
  exec: function(bindorfn){
    ...
    var args = Array.from(arguments).rest();
    return function(fn) {
  ...
  */
  decorate: function() {
    var decorators = Array.from(arguments), orig = resultFn = this, decorator;
  ...
  */
  comp: function() {
    var fns = Array.from(arguments), self = this;
    return function() {
      var temp = Array.from(fns);
      var args = Array.from(arguments), result = ($callable(self)) ? self.apply(this, args) : null, fn;
  ...
  */
  partial: function(bind) {
    var self = this;
    args = Array.from(arguments).rest();
    return function() {
  ...
  */
  curry: function(bind) {
    var self = this, arglist = Function.arglist(this), args = Array.from(arguments).rest();
    ...
    return function curryFn() {
      var fargs = argmerge(Array.from(arguments), args);
  ...
  */
  dispatch: function() {
    ...
    return function () {
      var args = Array.from(arguments).filter($notnull);
  ...
  */
  msg: function(methodName) {
    var rest = Array.from(arguments).rest();
    return function(obj) {
